...
class MapEventWarp extends MapEvent {
    ...
    public function execute(Server $server, WebSocketUser $user) {
        if($this->conditionsMet($server, $user)) {
            Helpers::$MAP_HELPER->warpUserToMap($user, $this->warpMap, $this->warpX, $this->warpY, (!is_null($this->warpDirection)) ? $this->warpDirection : $server->getAccountFromUser($user)->getCurrentCharacter()->getWorldPosition()->getDirection());
        }
    }
