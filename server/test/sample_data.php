...
Database::get()->getEntityManager()->transactional(
    function ($entityManager) {
        ...
        foreach ($accounts as $account) {
            $a = Account::findOneBy(array('login' =>  $account['login']));
            if ($a === null) {
                $a = new Account();
            ...
            foreach ($account['characters'] as $character) {
                ...
                //character
                $c = Character::findOneBy(array('name'    =>  $character['name']));
                if ($c === null) {
                    $c = new Character();
                }

                $c->setName($character['name']);
                $c->setCharset(Charset::findOneBy(array('name'    =>  $character['charset'])));
                $c->setAccount($a);

                //world position
                $wp = $c->getWorldPosition();
                if ($wp === null) {
